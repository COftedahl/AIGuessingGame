...
public class InnerLayer {
    ...
    public String toString(int numSpacesIndented) {
        ...
        String output = "";
        output += prefix + "<InnerLayer>\n  " + prefix;
        output += "<numNodes>" + numNodes + "</numNodes>\n  " + prefix;
        output += "<mappingConfig>" + mappingConfig.toString() + "</mappingConfig>\n  " + prefix;
        output += "<bias>" + Arrays.toString(bias).substring(1, Arrays.toString(bias).length() - 2) + "</bias>\n  " + prefix;
        output += "<weights>" + Arrays.deepToString(weights).replace('[',' ').replace(']', ' ').trim() + "</weights>\n" + prefix;
        output += "</InnerLayer>";
        return output;
    }
}
