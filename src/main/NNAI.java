...
public class NNAI {
    ...
    public void testGeneration(Game g) {
        ...
        for (ScoredNeuralNetwork scoredNet : nns) {
            sumScores = 0;
            numGames = 0;
            for (GameResultsImpl result : scoredNet.getResults()) {
                numGames += 1;
        ...
        sortNets();
        System.out.println("There were " + gamesPlayed + " games played in testing this generation" +
                " of " + numNNs + " NN's with each NN playing " + numGames + " games.");
    }
    ...
    public void evolveGeneration() {
        ...
        for (int currChild = 0; currChild < newNetArray.length; currChild += 1) {
            ...
            for (int i = 0; i < nns[indexOne].getNN().getInnerLayers().length; i += 1) {
            ...
            for (int i = 0; i < nns[indexOne].getNN().getInnerLayers().length; i += 1) {
                ...
                for (int j = 0; j < weightsOne.length; j += 1) {
                    for (int k = 0; k < weightsOne[j].length; k += 1) {
                        if (Math.random() > .5) {
            ...
            ScoredNeuralNetwork newScoredNet = new ScoredNeuralNetwork(scorer, 1, 1);
    ...
    public String toString(int numSpacesIndented) {
        ...
        String output = "";
        output += prefix + "<NNAI>\n  " + prefix;
        output += "<numGamesPerGen>" + numGamesPerGen + "</numGamesPerGen>\n  " + prefix;
        output += "<numNNs>" + numNNs + "</numNNs>\n  " + prefix;
        output += "<chanceToMutate>" + chanceToMutate + "</chanceToMutate>\n  " + prefix;
        output += "<mutationAmount>" + mutationAmount + "</mutationAmount>\n  " + prefix;
        output += "<chanceToExplore>" + chanceToExplore + "</chanceToExplore>\n  " + prefix;
        output += "<explorationDecayRate>" + explorationDecayRate + "</explorationDecayRate>\n  " + prefix;
        output += "<currGen>" + currGen + "</currGen>\n  " + prefix;
        output += "<numInputs>" + numInputs + "</numInputs>\n  " + prefix;
        output += "<numOutputs>" + numOutputs + "</numOutputs>\n";
        output += scorer.toString(numSpacesIndented + 2);
        output += "\n" + dumbNet.toString(numSpacesIndented + 2);
        output += "\n  <nns>";
        for (int i = 0; i < nns.length; i += 1) {
            output += "\n" + nns[i].toString(numSpacesIndented + 4);
        }
        output += "\n  </nns>\n</NNAI>";
        return output;
    }
